...
def get_filters():
    ...
    while True:
        city = input(
            "Input data City(Chicago, New York, Washington: ").lower()
        if city in cities:
            break
        else:
            print('Error input data city')

    # Get user to filter by month, day, or none.
    while True:
        choice = input("Do you want to filter data by month, day or not?\n").lower()
        if choice == 'month':
            month = input(
                "Input data month:  All, January, February, March, April, May, June: ").lower()
            day = 'all'
            if month in months:
                break
            else:
                print('Error input data month.')
        elif choice == 'day':
            day = input(
                "Input data weekday (All, Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday): ").lower()  # PB
            month = 'all'
            if day in days:
                break
            else:
                print('Error input data city day')
        elif choice == 'none':
            month = 'all'
            day = 'all'
            break
